# 14. Vehicle History & Title

Source: CARFAX Vehicle History Report (REF-022), run 2026-09-05, reference 20262471852110.

## Headline findings

> **This truck is a Ford manufacturer buyback.** On 2026-07-01 Ford Motor Company reacquired the vehicle "pursuant to applicable state consumer warranty laws or to promote customer satisfaction." It was retitled to the manufacturer in Michigan, sold at auction on 2026-08-25 with the buyback disclosed, and bought by the current owner on 2026-09-08. **The title carries a manufacturer buyback brand; that is permanent and unaffected by the repairs below.**

> **The two cited defects were repaired by Ford.** The reasons Ford gave for the reacquisition were:
>
> 1. The passenger-side window did not operate correctly.
> 2. The Cross Traffic / Blind Spot sensors did not operate correctly.
>
> **Ford repair documentation held by the owner (REF-026) records both as repaired.** This supersedes the earlier reading of this chapter, which noted only that CARFAX showed no corresponding repair - the CARFAX record ends at a lamp assembly on 2026-08-10 and does not reflect the manufacturer's own repair work during the reacquisition period. Manufacturer repairs on a reacquired vehicle are frequently absent from CARFAX.

## Status summary

| Item | Finding |
| --- | --- |
| Odometer (last reported) | 38,278 mi (2026-08-25) |
| Owners | 1 (commercial) |
| Title states | Tennessee, then Michigan |
| Accidents / damage | None reported |
| Structural damage | None reported |
| Airbag deployment | None reported |
| Odometer brands | No rollback; no problem (CARFAX guaranteed) |
| Damage brands (salvage/junk/flood/etc.) | No problem (CARFAX guaranteed) |
| Open recalls | None reported |
| Service history records | 15 |
| Average use | 16,783 mi/yr |
| CARFAX retail value | $50,480 |

## Warranty position

The window sticker (REF-003) recorded 3 yr/36,000 mi bumper-to-bumper and 5 yr/60,000 mi powertrain from an in-service date of roughly 2024-04-30.

| Coverage | Status at 38,278 mi |
| --- | --- |
| Bumper to bumper (3/36) | **Expired** - CARFAX confirms basic warranty expired |
| Powertrain (5/60) | Should remain active to 60,000 mi or 2029-04, subject to transfer rules |

Confirm remaining coverage with Ford by VIN. Buyback vehicles can carry different terms, and some states require a separate warranty on a reacquired vehicle.

## Ownership and title timeline

| Date | Mileage | Event |
| --- | --- | --- |
| 2024-04-23 | | Manufactured, shipped to original dealer (Friendship Ford, Bristol, TN) |
| 2024-04-25 | 5 | Pre-delivery inspection |
| 2024-04-26 | | Offered for sale |
| 2024-04-30 | | **Sold**; purchase reported to Tennessee MVD |
| 2024-05-15 | | Title issued, Blountville, TN - titled as commercial vehicle, color Gray (title #18200364148) |
| 2025-04-30 | | Registration renewed (TN) |
| 2026-05-22 | | Registration renewed (TN) |
| 2026-07-01 | | **VEHICLE REACQUIRED by Ford Motor Company** - window and Cross Traffic/Blind Spot sensor defects cited |
| 2026-07-08 | 38,255 | Title issued to manufacturer, Saginaw, MI (title #MI0030735498) |
| 2026-08-17 | | Online listing |
| 2026-08-25 | 38,278 | Auto auction - **manufacturer buyback disclosed by seller**; vehicle sold |
| 2026-08-25 | | Offered for sale by Northside Ford Lincoln, Effingham, IL |
| **2026-09-08** | 38,278 | **Purchased by current owner** |

### Dealer discrepancy - resolved

The conflict flagged in [chapter 1](01-vehicle-information.md) is explained. Both dealers are genuine and sequential:

- **Friendship Ford, Bristol, Tennessee** - original selling dealer on the window sticker (2024).
- **Northside Ford Lincoln, Effingham, Illinois** - current listing dealer following the 2026 buyback and auction.

## Service history

Fifteen dealer service records. Complete history in [logs/service-log.md](../logs/service-log.md); summary of servicing dealers:

| Dealer | Location | Visits |
| --- | --- | --- |
| Friendship Ford | Bristol, TN | 7 |
| Johnson City Ford | Johnson City, TN | 4 |
| Wallace Ford of Kingsport | Kingsport, TN | 3 |
| Don Franklin Hardin County Ford | Radcliff, KY | 1 |

Patterns worth noting:

- **Repeated brake and alignment attention.** Brakes checked at 10,914 / 11,108 / 20,571 / 21,321 / 23,725 / 35,930 mi; alignment performed at 17,704, 20,571, and 21,321 mi. Front wheel bearing(s)/hub(s) replaced at 11,108 mi.
- **Repeated battery/charging system checks** at 10,914 / 20,571 / 21,321 / 23,725 / 35,930 mi.
- **Electrical investigation late in life** - electrical system checked at 35,930 and 36,971 mi, outside mirror replaced/repaired and steering wheel centered at 35,930 mi. These immediately precede the buyback and are plausibly related to the cited defects.

## Verification list

Purchased 8 September 2026. Ford's repair paperwork (REF-026) records both cited defects as repaired, so items 1 and 2 are confirmation of work already done rather than fault-finding. The rest still stand:

1. **Passenger-side window** - full up/down cycle from both door and driver master switch.
2. **BLIS and Cross Traffic Alert** - confirm operation, including trailer-tow BLIS coverage. This matters directly for the fifth-wheel use case in [chapter 10](10-fifth-wheel-and-towing.md).
3. Pull all module DTCs and check for stored history relating to the sensor faults.
4. ~~Obtain the Ford repair orders from the buyback period~~ - **done**, held as REF-026. Scan them into `references/` to complete the record.
5. Confirm the title's brand and disclosure language as issued.
6. Confirm remaining powertrain coverage and any state-mandated buyback warranty, by VIN.
7. Verify no open recalls or Field Service Actions at time of sale.
